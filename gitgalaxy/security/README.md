# GitGalaxy: Security & Validation Models
...
[![Threat Hunting](https://img.shields.io/badge/Threat_Hunting-Structural_DNA-00BFFF.svg)](#)
...
Unlike traditional static analysis tools that look for specific CVEs or known vulnerable package versions, GitGalaxy looks for the *Structural DNA* of a threat. It measures the physical distance between an I/O input node and a dangerous execution command to calculate exact Weaponizable Surface Area.
...
* **`security_lens.py`:** The mathematical engine for threat detection. It houses the 13 raw "Passive Observers" (e.g., `sec_heat_triggers`, `sec_shadow_imports`, `sec_homoglyphs`). It uses C-backed Shannon Entropy math to catch obfuscated malware and sub-atomic XOR decryption loops hidden inside massive string literals.
