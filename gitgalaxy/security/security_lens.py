...
class SecurityLens:
    ...
    def __init__(self, policy=None):
        ...
        # THE FIX: Bounded, non-cross-line regex to prevent catastrophic backtracking.
        # Drops strings < 64 chars or > 1024 chars instantly at the C-engine level.
    ...
    def _calculate_shannon_entropy(self, data: str) -> float:
        """Calculates the Shannon Entropy of a string. Optimized Math."""
        if not data:
            return 0.0
        ...
        # THE FIX: math.log2(L) - sum(c * math.log2(c)) / L
        # Removes the division from inside the loop
    ...
    def scan_content(self, content: str, loc: int) -> dict:
        ...
        # THE FIX: O(1) Taint Slicer Offset Map
        ...
        for key, regex in self.THREAT_SIGNATURES.items():
            ...
            if counts[key] > 0:
                for match in regex.finditer(safe_content):
                    ...
                    # Store exact line indexes of critical threats for the Taint Slicer
                    if not is_auto_gen and key in {"io", "danger", "llm_hooks", "db_hooks"}:
        ...
        if is_auto_gen:
            ...
            for match in self.string_extractor.finditer(safe_content):
                ...
                # Ensure it's dense data, not prose (spaces check)
        ...
        # ---> 4. N-DIMENSIONAL TAINT ANALYSIS (O(H) Offset Mapper) <---
        taint_hits = 0
        ...
        if (has_global_io or has_global_llm) and (has_global_danger or has_global_llm or has_global_db) and not is_auto_gen:
            ...
            # THE FIX: Iterate ONLY over the specific lines that triggered a threat!
