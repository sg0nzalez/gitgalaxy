...
class GuideStarLens:
    ...
    def get_intent_status(self, path: Union[str, Path]) -> Tuple[bool, Dict[str, Any]]:
        ...
        if not prior:
            ...
            if parts.intersection(self.INTENT_BIASED_SECTORS):
                return True, {"lang_id": "unknown", "intensity": 0.75, "source_proof": "Sector Bias"}
    ...
    def _parse_package_json(self, path: Path):
        ...
        try:
            with open(path, 'r', encoding='utf-8') as f:
                ...
                # Main Entry Point
                if 'main' in data:
                    self._inject_prior(data['main'], 'javascript', 0.95, "Manifest Entry (package.json:main)")
                ...
                if isinstance(bins, str):
                    ...
                    for b_path in bins.values():
                        self._inject_prior(b_path, 'javascript', 0.95, "Manifest Binary (package.json:bin)")
                ...
                for name, cmd in scripts.items():
                    ...
                    for f in files:
                        self._inject_prior(f, 'javascript', 0.85, f"Manifest Script (package.json:scripts:{name})")
    ...
    def _extract_execution_triggers(self, text: str):
        ...
        exec_matches = re.findall(r'(\.\/|python3?\s+|node\s+|sh\s+|bash\s+|cargo\s+run\s+|go\s+run\s+)([a-zA-Z0-9_\-\./]+)', text)
