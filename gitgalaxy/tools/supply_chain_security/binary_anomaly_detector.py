...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Binary Anomaly Detector: Entropy & Magic Byte Scanner"
    )
    ...
    print(
        "   - Embedded Execution Headers (e.g., executable signatures within static data)"
    )
    print(
        "   - High-Entropy Payloads (e.g., packed executables or obfuscated bitwise loops)"
    )
    ...
    for file_path, rel_path_str, ext, is_whitelisted in files_to_deep_scan:
        try:
            ...
            if sec_results["counts"].get("bitwise_hits", 0) > 0:
                has_anomaly = True
                anomaly_msgs.append(
                    "Obfuscated bitwise operations (XOR loops) detected"
                )
    ...
    if anomalies_found > 0:
        print(
            f" [BLOCKING ACTION] {anomalies_found} structural anomalies detected. Failing pipeline."
        )
        print(
            " TIP: Entropy-based detection naturally flags compressed or dense data."
        )
        print(
