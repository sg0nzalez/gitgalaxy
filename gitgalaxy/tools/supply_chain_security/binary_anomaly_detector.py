...
try:
    from gitgalaxy.standards.gitgalaxy_config import (
        ...
        XRAY_BYPASS_EXTENSIONS, XRAY_BYPASS_PATHS
    )
except ImportError:
...
def main():
    ...
    for root, dirs, files in os.walk(target_path):
        ...
        for file in files:
            ...
            if "/test/" in rel_path_str.lower() or "/tests/" in rel_path_str.lower() or "phpunit" in rel_path_str.lower():
                is_whitelisted = True
    ...
    print("   - Parasitic Execution Headers (e.g., executable logic buried in data blobs)")
    print("   - High-Entropy Encrypted Payloads (e.g., packed malware or sub-atomic XOR loops)")
    ...
    for file_path, rel_path_str, ext, is_whitelisted in files_to_deep_scan:
        try:
            ...
            if sec_results["counts"].get("bitwise_hits", 0) > 0:
                has_anomaly = True
                anomaly_msgs.append("Sub-atomic decryption routines (XOR loops) detected")
    ...
    if anomalies_found > 0:
        print(f" ❌ TRIAGE ALERT: {anomalies_found} structural anomalies detected. Blocking commit/PR.")
        print(f" 💡 TIP: X-Ray uses entropy math which naturally flags compression and dense data.")
        print(f"         - If safe extension (e.g., .gz, .json): Add to XRAY_BYPASS_EXTENSIONS")
        print(f"         - If safe specific file: Add to XRAY_BYPASS_PATHS")
        print(f"         - Edit these inside: gitgalaxy/standards/gitgalaxy_config.py")
        sys.exit(1)
    else:
