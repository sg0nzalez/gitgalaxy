...
def generate_java_entity(schema_json: dict, package_name: str) -> str:
    ...
    for col_name, col_data in properties.items():
        ...
        reserved_vars = {
            "class", "static", "public", "private", "protected", "return", "new", "system", 
            "default", "enum", "interface", "void", "try", "catch", "finally", "import", 
            "package", "super", "this", "const", "goto", 
            "byte", "int", "char", "short", "long", "float", "double", "boolean", "null", "true", "false"
        }
        ...
        if 'occurs' in constraints:
            ...
            java.append(f'    @ElementCollection')
            java.append(f'    @CollectionTable(name = "{table_name}_{col_name.lower()}", joinColumns = @JoinColumn(name = "{table_name.lower()}_id"))')
