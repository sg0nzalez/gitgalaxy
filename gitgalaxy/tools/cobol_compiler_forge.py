        ...
        if 'COPY ' in upper_line and not (len(line) > 6 and line[6] == '*'):
            ...
            if match:
                ...
                if copy_file:
                    ...
                    inlined_text = flatten_copybooks(copy_file.read_text(errors='ignore'), base_dir, current_depth + 1)
...
def extract_intent(source_text: str) -> tuple:
    ...
    if id_match: prog_id = id_match.group(1).strip()

    file_matches = re.finditer(r'SELECT\s+[A-Z0-9\-]+\s+ASSIGN\s+TO\s+([A-Z0-9\-]+)', source_text, re.IGNORECASE)
...
def generate_build_jcl(source_text: str, prog_name: str, files: set, dialect: str) -> str:
    ...
    jcl.append(f"//* ==========================================================")
    jcl.append(f"//* PHASE 1: INFRASTRUCTURE PROVISIONING (IEFBR14)")
    jcl.append(f"//* ==========================================================")
    jcl.append(f"//ALLOC EXEC PGM=IEFBR14")    
    
    jcl.append(f"//LOADLIB  DD DSN=HERC01.LOADLIB,DISP=(MOD,CATLG,DELETE),")
    jcl.append(f"//            UNIT=SYSDA,SPACE=(CYL,(5,5,10)),")
    ...
    jcl.append(f"//* ==========================================================")
    jcl.append(f"//* PHASE 2: IBM COMPILER & LINKAGE EDITOR")
    jcl.append(f"//* ==========================================================")
    
    # --- THE DIALECT SWITCH ---
    if dialect == "COBOL-85":
        jcl.append(f"//* 🚨 DIALECT SENSOR: COBOL-85+ DETECTED 🚨")
        jcl.append(f"//* ROUTING TO MODERN ENTERPRISE COMPILER (IGYWCL)")
        jcl.append(f"//COMPILE  EXEC IGYWCL")
    else:
        jcl.append(f"//* 🟢 DIALECT SENSOR: COBOL-74 DETECTED")
        jcl.append(f"//* ROUTING TO LEGACY OS/VS COMPILER (COBUCL)")
        jcl.append(f"//COMPILE  EXEC COBUCL")

    jcl.append(f"//COB.SYSIN DD *")
    jcl.extend(source_text.rstrip().split('\n'))
    jcl.append(f"/*")
    
    jcl.append(f"//* ==========================================================")
    jcl.append(f"//* PHASE 3: SAVE BINARY MODULE TO HERC01.LOADLIB")
    jcl.append(f"//* ==========================================================")
    jcl.append(f"//LKED.SYSLIB  DD DSN=SYS1.COBLIB,DISP=SHR")
