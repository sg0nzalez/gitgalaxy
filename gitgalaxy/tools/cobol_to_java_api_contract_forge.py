...
def generate_rest_controller(ir_state: dict, package_name: str) -> str:
    ...
    if is_batch:
        # --- BATCH PARADIGM ---
        java.append(f"    @PostMapping(value = \"/execute-batch\", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)")
        ...
        java.append("    ) {")
        java.append(f"        // ⚠️ BATCH PARADIGM DETECTED")
        java.append(f"        // Pass the InputStream directly to the Service layer.")
        java.append(f"        {service_var}Service.execute{camel_prog}(/* pass streams here */);\n")
        
    else:
        # --- TRANSACTIONAL PARADIGM ---
        java.append(f"    @PostMapping(\"/execute\")")
