...
> * 🟦 **IRONCLAD (Score 15-20):** Fortified. The code is heavily backed by internal assertions, mocks, or a dedicated sibling test suite. *(Note: The physics engine enforces a hard minimum floor of 15.0, acknowledging that no code is 100% perfectly safe).*
...
$$\text{InternalDensity} = \left( \frac{\text{test\_hits} \times 5.0}{\max(\text{LOC}, 1)} \right) \times 100.0$$
$$\text{TotalDensity} = \text{InternalDensity} + \text{SiblingBonus}$$
...
We multiply the result by the inverted Fidelity score ($2.0 - \text{Fc}$). If the file size exceeds the `MASSIVE_FILE_THRESHOLD` (300 lines), we calculate and add a structural mass penalty, capping the final calculation between the $15.0$ risk floor and $100.0$ maximum.
