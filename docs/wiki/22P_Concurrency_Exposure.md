...
We determine what percentage of the code is dedicated to managing time. We add a fraction of $\text{Irc}$ ($0.1\times$) to penalize implicit safety gaps without creating noise.

$$\text{WeightedHits} = \text{AsyncHits} + (\text{Irc} \times 0.1)$$
$$\text{Density} = \left( \frac{\text{WeightedHits}}{\max(\text{LOC}, 1)} \right) \times 100.0$$
...
$$\text{RawScore} = \frac{100.0}{1 + e^{-0.4 \times (\text{Density} - \text{Threshold})}}$$
$$\text{FinalScore} = \min(\text{RawScore}, 100.0)$$
