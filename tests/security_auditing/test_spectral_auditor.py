...
def test_auditor_consensus_engine(auditor):
    ...
    assert (
        mystery_file["lang_id"] == "cpp"
    ), "Failed to inherit the ecosystem consensus!"
    assert (
        mystery_file["telemetry"]["identity_lock_tier"] == 2
...
def test_auditor_supernova_guard(auditor):
    ...
    assert len(unparsable) == 1
    assert (
        "Supernova Guard" in unparsable[0]["reason"]
...
def test_auditor_quarantine_guard(auditor):
    ...
    assert (
        verified[0].get("is_quarantined") is True
