...
def guidestar(tmp_path):
    ...
    with patch('gitgalaxy.core.guidestar_lens.GuideStarLens._gs_config', MOCK_GUIDESTAR_CONFIG):
        with patch('gitgalaxy.core.guidestar_lens.GuideStarLens.MANIFEST_MAP', MOCK_GUIDESTAR_CONFIG["MANIFEST_MAP"]):
            with patch('gitgalaxy.core.guidestar_lens.GuideStarLens.INTENT_BIASED_SECTORS', set(MOCK_GUIDESTAR_CONFIG["INTENT_BIASED_SECTORS"])):
...
def test_guidestar_manifest_and_ai_detection(guidestar, tmp_path):
    ...
    pkg_data = {
        ...
        "dependencies": {
            "langchain": "^0.0.1" # The AI trigger keyword!
        }
    }
