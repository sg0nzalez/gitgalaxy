...
def test_process_payload_integration(tmp_path):
    ...
    assert mgr.get_orphaned_vars("MAINPGM") == {
        "DEAD-VAR"
...
def test_main_full_orchestration(tmp_path, capsys):
    ...
    assert (
        clean_dir / "01_zero_trust_jcls" / "PRG1.jcl"
    ).exists(), "Failed to write JCL artifacts!"
    assert (
        clean_dir / "02_cloud_schemas" / "PRG1_schema.sql"
    ).exists(), "Failed to write SQL schema artifacts!"
    assert (
        clean_dir / "02_cloud_schemas" / "PRG1_schema.json"
    ).exists(), "Failed to write JSON schema artifacts!"
    assert (
        clean_dir / "04_ir_state_dumps" / "PRG1_ir.json"
    ).exists(), "Failed to write IR State dumps!"
    assert (
        clean_dir / "05_microservice_slices" / "PRG1_slice.json"
    ).exists(), "Failed to write Microservice Slices!"
    ...
    assert (
        "HONESTY FLAG: Hardcoded IP" in audit_text
    ), "Failed to aggregate System Limit honesty flags!"
    assert (
        "Unresolved Dynamic CALL to: EXTERNAL_CALL" in audit_text
    ), "Failed to aggregate Lineage honesty flags!"
    assert (
        "AI Agent Job Tickets Generated : 3" in audit_text
    ), "Failed to aggregate Agent Job counts!"
    assert (
        "Bloat Removed: ~50 Lines" in audit_text
