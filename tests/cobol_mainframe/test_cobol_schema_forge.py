...
def test_forge_schemas_e2e(tmp_path):
    ...
    assert (
        "CREATE TABLE ACCOUNT_RECORD" in sql_ddl
    ...
    assert (
        "COMP-3 (Packed Decimal)" in sql_ddl
    ), "Failed to tag the legacy COMP-3 footprint!"
    assert (
        "WARNING: OCCURS DEPENDING ON detected. Use JSONB." in sql_ddl
