...
import gitgalaxy_standards_v011 as scanning_config
...
class Orchestrator:
    ...
    def run_mission(self, output_file: str = "galaxy.json"):
        ...
        try:
            ...
            logger.info(f"--- MISSION_SUCCESS: {stars_mapped_count} stars mapped in {session_meta['duration_seconds']}s ---")
            logger.info(f"--- ARCHIVE_SEALED: {output_file} & {audit_output} ---")
                ...
                for future in done:
                    ...
                    try:
                        ...
                        if status == "success":
                            ...
                            logger.warning(f"SINGULARITY_BYPASS: '{rel_path}' lacks structural integrity. Relegating to Dark Matter.")
                            self.singularity_candidates.append({
                                "path": rel_path,
                                "reason": res["reason"],
    ...
    def _record_anomaly(self, path: Union[str, Path], message: str):
        ...
        name = Path(path).name
        logger.warning(f"ANOMALY: {name} | {message}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # ---------------------------------------------------------
        # Dynamically determine the output filename and project name based on the target directory
        ...
        # Deepcopy protects the pristine dictionary in memory
        merged_langs = copy.deepcopy(base_langs) 
        
        # If the target project has a registered Dialect, inject it now
        if project_name in project_overrides:
            ...
            for lang, overrides in dialect_dict.items():
                ...
                if lang in merged_langs:
                    # 1. Handle Extension Overrides
                    if "extensions" in overrides:
                        merged_langs[lang]["extensions"] = overrides["extensions"]
                        logging.info(f"   -> Patched '{lang}' extensions.")
                        
                    # 2. Handle Regex Rule Overrides
                    ...
                    if rules_patch and 'rules' in merged_langs[lang]:
                        ...
                        logging.info(f"   -> Patched '{lang}' geometry rules.")

        # ---------------------------------------------------------
        # 3. Assemble the Final Configuration
        # ---------------------------------------------------------
        # Assemble Config using the newly patched language definitions
        full_config = {
            ...
            "DOCUMENTATION_LANGUAGES": getattr(scanning_config, "DOCUMENTATION_LANGUAGES", set()) # <-- Updated name
        }
        ...
        # Bypasses Python's garbage collector to prevent end-of-run hangs on massive datasets.
        os._exit(0)
